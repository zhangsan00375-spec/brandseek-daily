...
def send_via_resend(to_email: str, html_content: str, api_key: str):
    ...
    payload = json.dumps({"from": "BrandSeek <daily@brandseek.io>", "to": [to_email], "subject": f"【BrandSeek日报】{datetime.now().strftime('%Y-%m-%d')} 品牌资产动态", "html": html_content}).encode("utf-8")
...
def main():
    ...
    if not to_email:
        print("Warning: TO_EMAIL not set, preview only"); return
    if api_key:
