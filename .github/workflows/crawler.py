    ...
    def crawl(self) -> List[AssetItem]:
        ...
        for kw in self.KEYWORDS:
            try:
                ...
                req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36", "Referer": "https://auction.jd.com/"})
                with urllib.request.urlopen(req, timeout=15) as resp:
                    data = json.loads(resp.read().decode("utf-8"))
                for item in data.get("data", {}).get("list", []):
                    title = item.get("title", "")
                    if self._is_brand_related(title):
                        items.append(AssetItem(title=title, source="京东拍卖", status=self._map_status(item.get("status", "")), price=f"¥{item.get('startPrice', '0'):,.0f}", url=f"https://auction.jd.com/{item.get('id', '')}", date=item.get("startTime", "")[:10], description=item.get("description", "")[:200], region=item.get("region", "")))
            except Exception as e:
                print(f"[JD] '{kw}' failed: {e}")
        return items[:10]
    def _is_brand_related(self, title: str) -> bool:
        return any(kw in title for kw in ["商标", "品牌", "字号", "专利", "加盟", "特许", "股权", "老字号"])
    def _map_status(self, status: str) -> str:
        return {"1": "即将开拍", "2": "法拍中", "3": "已成交", "4": "流拍"}.get(status, "拍卖中")
    ...
    def crawl(self) -> List[AssetItem]:
        ...
        for kw in self.KEYWORDS:
            try:
                ...
                json_matches = re.findall(r'"id":"(\d+)".*?"title":"([^"]*商标[^"]*)".*?"startPrice":"?([\d.]+)"?.*?"status":"?(\d)"?.*?"startTime":"([^"]*)"', content, re.DOTALL)
                for item_id, title, price, status, start_time in json_matches:
            ...
            try:
                ...
                for item in root.findall(".//item")[:5]:
                    ...
                    if title_el is not None and link_el is not None:
                        ...
                        items.append(NewsItem(title=title_el.text or "", source=src_el.text if src_el is not None else "Google News", date=date_str, summary=summary, url=link_el.text or "", category=self._categorize(title_el.text or "")))
            except Exception as e:
                print(f"[NEWS] '{kw}' failed: {e}")
        return self._deduplicate(items)[:15]
    def _categorize(self, title: str) -> str:
        if "成交" in title or "拍出" in title: return "拍卖成交"
        if "破产" in title: return "破产拍卖"
        if "转让" in title: return "品牌转让"
        return "行业动态"
...
class BrandSeekCrawler:
    ...
    def run(self) -> dict:
        ...
        for name, crawler in [("jd", self.crawlers["jd"]), ("ali", self.crawlers["ali"])]:
            print(f"[Crawl] {name}..."); items = crawler.crawl(); all_assets.extend(items); stats[name] = len(items); print(f"  -> {len(items)} items")
        print("[Crawl] court..."); court_items = self.crawlers["court"].crawl(); all_assets.extend(court_items); stats["court"] = len(court_items); print(f"  -> {len(court_items)} items")
        print("[Crawl] news..."); news_items = self.crawlers["news"].crawl(); all_news.extend(news_items); stats["news"] = len(news_items); print(f"  -> {len(news_items)} items")
        all_assets = self._dedup_assets(all_assets); all_news = self._dedup_news(all_news)
        total = len(all_assets) + len(all_news); print(f"\nDone: {len(all_assets)} assets + {len(all_news)} news = {total} total\n")
        return {"assets": [asdict(a) for a in all_assets], "news": [asdict(n) for n in all_news], "stats": stats, "timestamp": datetime.now().isoformat()}
    def _dedup_assets(self, items: List[AssetItem]) -> List[AssetItem]:
        seen, result = set(), []
        for item in items:
            key = item.title[:20]
